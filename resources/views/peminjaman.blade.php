...
<script>
    ...
    document.getElementById('peminjamanForm').addEventListener('submit', function(e) {
        ...
        const isFakultas = {
            {
                auth() - > user() - > isFakultas ? 'true' : 'false'
            }
        };
...
<script>
    function calculateEstimatedTarif() {
        ...
        const isHourlyRate = {
            {
                isset($ruangan) ? ($ruangan - > is_hourly_rate ? 'true' : 'false') : ($sarana - > is_hourly_rate ? 'true' : 'false')
            }
        };
        const hoursPerUnit = {
            {
                isset($ruangan) ? ($ruangan - > hours_per_unit ?? 0) : ($sarana - > hours_per_unit ?? 0)
            }
        };
