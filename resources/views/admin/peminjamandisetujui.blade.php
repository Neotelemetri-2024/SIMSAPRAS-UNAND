...
<div class="p-4 sm:p-6">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <div class="p-5">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($peminjamanDisetujui as $index => $item)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">
                                {{ $loop->iteration + ($peminjamanDisetujui->currentPage() - 1) * $peminjamanDisetujui->perPage() }}
                            </td>
        ...
        <div class="px-6 py-4">
            @if ($peminjamanDisetujui->hasPages())
            <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
                <div class="flex justify-between flex-1 sm:hidden">
                    @if ($peminjamanDisetujui->onFirstPage())
                    <span
                    ...
                    @else
                    <a href="{{ $peminjamanDisetujui->previousPageUrl() }}"
                    ...
                    @if ($peminjamanDisetujui->hasMorePages())
                    <a href="{{ $peminjamanDisetujui->nextPageUrl() }}"
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700 leading-5">
                            Showing
                            <span class="font-medium">{{ $peminjamanDisetujui->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $peminjamanDisetujui->lastItem() }}</span>
                            of
                            <span class="font-medium">{{ $peminjamanDisetujui->total() }}</span>
                            results
                        </p>
                    </div>
                    ...
                    <div>
                        <span class="relative z-0 inline-flex shadow-sm rounded-md">
                            ...
                            @else
                            <a href="{{ $peminjamanDisetujui->previousPageUrl() }}" rel="prev"
                            ...
                            @foreach ($peminjamanDisetujui->getUrlRange(1, $peminjamanDisetujui->lastPage()) as $page => $url)
                            @if ($page == $peminjamanDisetujui->currentPage())
                            ...
                            {{-- Next Page Link --}}
                            @if ($peminjamanDisetujui->hasMorePages())
                            <a href="{{ $peminjamanDisetujui->nextPageUrl() }}" rel="next"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (openModalId) {
            const modal = document.getElementById(`editModalDisetujui${openModalId}`);
            if (modal) {
