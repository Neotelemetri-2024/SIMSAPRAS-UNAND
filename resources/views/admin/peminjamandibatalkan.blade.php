...
<div class="p-4 sm:p-6">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <div class="p-5">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($peminjamanDibatalkan as $index => $item)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">
                                {{ $loop->iteration + ($peminjamanDibatalkan->currentPage() - 1) * $peminjamanDibatalkan->perPage() }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <button data-modal-target="editModalDibatalkan{{ $item->id }}"
                                    data-modal-toggle="editModalDibatalkan{{ $item->id }}"
        ...
        <div class="px-6 py-4">
            @if ($peminjamanDibatalkan->hasPages())
            <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
                <div class="flex justify-between flex-1 sm:hidden">
                    @if ($peminjamanDibatalkan->onFirstPage())
                    <span
                    ...
                    @else
                    <a href="{{ $peminjamanDibatalkan->previousPageUrl() }}"
                    ...
                    @if ($peminjamanDibatalkan->hasMorePages())
                    <a href="{{ $peminjamanDibatalkan->nextPageUrl() }}"
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700 leading-5">
                            Showing
                            <span class="font-medium">{{ $peminjamanDibatalkan->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $peminjamanDibatalkan->lastItem() }}</span>
                            of
                            <span class="font-medium">{{ $peminjamanDibatalkan->total() }}</span>
                            results
                        </p>
                    </div>
                    ...
                    <div>
                        <span class="relative z-0 inline-flex shadow-sm rounded-md">
                            ...
                            @else
                            <a href="{{ $peminjamanDibatalkan->previousPageUrl() }}" rel="prev"
                            ...
                            @foreach ($peminjamanDibatalkan->getUrlRange(1, $peminjamanDibatalkan->lastPage()) as $page => $url)
                            @if ($page == $peminjamanDibatalkan->currentPage())
                            ...
                            {{-- Next Page Link --}}
                            @if ($peminjamanDibatalkan->hasMorePages())
                            <a href="{{ $peminjamanDibatalkan->nextPageUrl() }}" rel="next"
...
@foreach ($peminjamanDibatalkan as $item)
<div id="editModalDibatalkan{{ $item->id }}" tabindex="-1" aria-hidden="true"
    class="fixed inset-0 z-[60] hidden overflow-hidden" data-modal-backdrop="static">
    <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity" data-modal-hide="editModalDibatalkan{{ $item->id }}"></div>
            ...
            <div class="relative flex flex-col max-h-[90vh] bg-white rounded-lg shadow">
                <div class="sticky top-0 z-10 flex items-start justify-between p-5 border-b rounded-t bg-gray-50">
                    ...
                    <button type="button"
                        ...
                        data-modal-hide="editModalDibatalkan{{ $item->id }}">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (openModalId) {
            const modal = document.getElementById(`editModalDibatalkan${openModalId}`);
            if (modal) {
    ...
    window.addEventListener('click', function(event) {
        const modals = document.querySelectorAll('[id^="editModalDibatalkan"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const modals = document.querySelectorAll('[id^="editModalDibatalkan"]');
        modals.forEach(modal => {
            const id = modal.id.replace('editModalDibatalkan', '');
