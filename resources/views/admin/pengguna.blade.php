    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <div class="p-5">
            ...
            <div class="mb-4 justify-end flex">
                <form method="GET" action="{{ route('pengguna.index') }}" id="roleFilterForm">
                    <div class="flex items-center gap-3">
                        <label for="roleFilter" class="text-sm font-medium text-gray-900 dark:text-white">Filter Role:</label>
                        <select id="roleFilter" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-48 p-2.5" onchange="this.form.submit()">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($item->role == 'user') bg-blue-100 text-blue-800
                                    @elseif($item->role == 'admin') bg-green-100 text-green-800
                                    @elseif($item->role == 'pimpinan') bg-purple-100 text-purple-800
                                    @endif">
                                    {{ ucfirst($item->role) }}
                                </span>
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                                @if(request('role'))
                                    Tidak ada data untuk role {{ ucfirst(request('role')) }}
                                @else
...
<div id="editModal{{ $item->hashed_id }}" tabindex="-1" aria-hidden="true" class="fixed inset-0 z-[60] hidden overflow-y-auto overflow-x-hidden" data-modal-backdrop="static">
    ...
    <div class="relative w-full max-w-2xl bg-white rounded-lg shadow dark:bg-gray-700 m-4">
        ...
        <form action="{{ route('pengguna.update', $item->hashed_id) }}" method="POST" class="edit-form">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-6">
                <div>
