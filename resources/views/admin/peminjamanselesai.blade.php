...
<div class="p-4 sm:p-6">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        <div class="p-5 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            ...
            <button id="exportButton" type="button" data-modal-target="exportModal" data-modal-toggle="exportModal"
                class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2.5 inline-flex items-center">
        ...
        <div class="p-5">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($peminjamanSelesai as $index => $item)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">
                                {{ $loop->iteration + ($peminjamanSelesai->currentPage() - 1) * $peminjamanSelesai->perPage() }}
                            </td>
        ...
        <div class="px-6 py-4">
            ...
            <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
                <div class="flex justify-between flex-1 sm:hidden">
                    @if ($peminjamanSelesai->onFirstPage())
                    <span
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700 leading-5">
                            Showing
                            <span class="font-medium">{{ $peminjamanSelesai->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $peminjamanSelesai->lastItem() }}</span>
                            of
                            <span class="font-medium">{{ $peminjamanSelesai->total() }}</span>
                            results
                        </p>
                    </div>
                    ...
                    <div>
                        <span class="relative z-0 inline-flex shadow-sm rounded-md">
                            ...
                            @foreach ($peminjamanSelesai->getUrlRange(1, $peminjamanSelesai->lastPage()) as $page => $url)
...
<div id="exportModal" tabindex="-1" aria-hidden="true" class="fixed inset-0 z-50 hidden overflow-y-auto overflow-x-hidden">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (openModalId) {
            const modal = document.getElementById(`editModalSelesai${openModalId}`);
            if (modal) {
    ...
    window.addEventListener('click', function(event) {
        const modals = document.querySelectorAll('[id^="editModalSelesai"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const modals = document.querySelectorAll('[id^="editModalSelesai"]');
        modals.forEach(modal => {
            const id = modal.id.replace('editModalSelesai', '');
