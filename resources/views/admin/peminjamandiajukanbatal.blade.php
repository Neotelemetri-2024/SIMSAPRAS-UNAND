...
<div class="p-4 sm:p-6">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <div class="p-5">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($peminjamanDiajukanbatal as $index => $item)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4">
                                {{ $loop->iteration + ($peminjamanDiajukanbatal->currentPage() - 1) * $peminjamanDiajukanbatal->perPage() }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center bg-purple-100 text-purple-800 text-sm font-medium px-3 py-1.5 rounded-full dark:bg-purple-900 dark:text-purple-300">
                            ...
                            <td class="px-6 py-4">
                                <button data-modal-target="editModalDiajukanbatal{{ $item->id }}"
                                    ...
                                    class="inline-flex items-center justify-center px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-gren-700 focus:ring-4 focus:ring-gren-200">
        ...
        <div class="px-6 py-4">
            @if ($peminjamanDiajukanbatal->hasPages())
            <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
                <div class="flex justify-between flex-1 sm:hidden">
                    @if ($peminjamanDiajukanbatal->onFirstPage())
                    <span
                    ...
                    @else
                    <a href="{{ $peminjamanDiajukanbatal->previousPageUrl() }}"
                    ...
                    @if ($peminjamanDiajukanbatal->hasMorePages())
                    <a href="{{ $peminjamanDiajukanbatal->nextPageUrl() }}"
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700 leading-5">
                            Showing
                            <span class="font-medium">{{ $peminjamanDiajukanbatal->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $peminjamanDiajukanbatal->lastItem() }}</span>
                            of
                            <span class="font-medium">{{ $peminjamanDiajukanbatal->total() }}</span>
                            results
                        </p>
                    </div>
                    ...
                    <div>
                        <span class="relative z-0 inline-flex shadow-sm rounded-md">
                            ...
                            @else
                            <a href="{{ $peminjamanDiajukanbatal->previousPageUrl() }}" rel="prev"
                            ...
                            @foreach ($peminjamanDiajukanbatal->getUrlRange(1, $peminjamanDiajukanbatal->lastPage()) as $page => $url)
                            @if ($page == $peminjamanDiajukanbatal->currentPage())
                            ...
                            {{-- Next Page Link --}}
                            @if ($peminjamanDiajukanbatal->hasMorePages())
                            <a href="{{ $peminjamanDiajukanbatal->nextPageUrl() }}" rel="next"
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <div class="space-y-3">
                                ...
                                @else
                                <p class="text-gray-500 italic">Tidak ada alasan pembatalan yang dicantumkan.</p>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <select id="statusSelect{{ $item->id }}" name="status"
                                            ...
                                            <option value="diajukan">Tolak Pembatalan</option>
                                        </select>
                                    </div>
                                    ...
                                    <div id="feedbackForm{{ $item->id }}" class="hidden">
                                        ...
                                        <textarea name="alasanTolakBatal" rows="3"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
        ...
        <div class="relative w-full max-w-md">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex items-center justify-end p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    ...
                    <button data-modal-hide="batalModalDiajukanbatal{{ $item->id }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (openModalId) {
            const modal = document.getElementById(`editModalDiajukanBatal${openModalId}`);
            if (modal) {
