            ...
            <div class="w-48">
               <select name="sarana"
                   ...
                   @foreach($sarana as $sar)
                       <option value="{{ $sar->hashed_id }}" {{ request('sarana') == $sar->id ? 'selected' : '' }}>
                           {{ $sar->nama }}
                       </option>
                   @endforeach
               </select>
                  ...
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex space-x-2">
                            ...
                            <button onclick="confirmDelete('{{ route('penjaga.destroy', $item->hashed_id) }}')"
             ...
             <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="idSarana" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                       ...
                       @foreach($sarana as $sar)
                       <option value="{{ $sar->hashed_id }}">{{ $sar->nama }}</option>
                       @endforeach
                    </select>
                 </div>
             </div>
 ...
 <div id="editModal{{ $item->hashed_id }}" tabindex="-1" aria-hidden="true" class="fixed inset-0 z-[60] hidden overflow-y-auto overflow-x-hidden" data-modal-backdrop="static">
    ...
    <div class="relative w-full max-w-2xl max-h-full">
       <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <form id="editPenjagaForm" action="{{ route('penjaga.update', $item->hashed_id) }}" method="POST">
             @csrf
             @method('PUT')
             <div class="p-6 space-y-6">
                <div>
                ...
                <div>
                    ...
                    <select name="idSarana" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
    @foreach($sarana as $sar)
    <option value="{{ $sar->hashed_id }}" {{ $sar->id == $item->idSarana ? 'selected' : '' }}>
        {{ $sar->nama }}
    </option>
    @endforeach
</select>
