                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Riwayat Peminjaman</span>
                        </div>
                    </li>
                </ol>
            </nav>
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 items-end">
                            <div class="flex-1">
                                ...
                                <select name="status"
                                    ...
                                    <option value="diajukanbatal" {{ $status == 'diajukanbatal' ? 'selected' : '' }}>
                                        Diajukan Batal</option>
                                </select>
                            </div>
                            <div class="sm:w-48">
                            ...
                            @forelse($peminjaman as $pinjam)
                                <tr class="bg-white hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 text-gray-600">
                                        Rp{{ number_format($pinjam->totalTarif, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4">
                                        @if ($pinjam->status == 'diproses')
                                            @if ($pinjam->buktiPembayaran)
                                                <span
                                                    class="bg-indigo-50 text-indigo-700 text-xs font-medium px-3 py-1.5 rounded-full border border-indigo-200 flex items-center w-fit gap-1">
                                            ...
                                            @else
                                                <span
                                                    class="bg-yellow-50 text-yellow-700 text-xs font-medium px-3 py-1.5 rounded-full border border-yellow-200 flex items-center w-fit gap-1">
                                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
                                        ...
                                        @elseif($pinjam->status == 'diajukan')
                                            <span
                                                class="bg-blue-50 text-blue-700 text-xs font-medium px-3 py-1.5 rounded-full border border-blue-200 flex items-center w-fit gap-1">
                                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path
                                        ...
                                        @elseif($pinjam->status == 'ditolak')
                                            <span
                                                class="bg-red-50 text-red-700 text-xs font-medium px-3 py-1.5 rounded-full border border-red-200 flex items-center w-fit gap-1">
                                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                        ...
                                        @elseif($pinjam->status == 'disetujui')
                                            <span
                                                class="bg-emerald-50 text-emerald-700 text-xs font-medium px-3 py-1.5 rounded-full border border-emerald-200 flex items-center w-fit gap-1">
                                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                        ...
                                        @elseif($pinjam->status == 'diajukanbatal')
                                            <span
                                                class="bg-purple-50 text-purple-700 text-xs font-medium px-3 py-1.5 rounded-full border border-purple-200 flex items-center w-fit gap-1">
                                                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none">
                                                    <circle cx="12" cy="12" r="10" fill="none"
                                                        stroke="currentColor" stroke-width="2" />
                                                    <path d="M8.5 8.5L15.5 15.5M15.5 8.5L8.5 15.5" stroke="currentColor"
                                        ...
                                        @elseif($pinjam->status == 'dibatalkan')
                                            <span
                                                class="bg-gray-50 text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full border border-gray-200 flex items-center w-fit gap-1">
                                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                                        d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z"
                                                        clip-rule="evenodd" />
        ...
        @foreach ($peminjaman as $pinjam)
            <div id="detailModal{{ $pinjam->id }}" tabindex="-1" aria-hidden="true"
                ...
                <div class="fixed inset-0 bg-black bg-opacity-70 transition-opacity"
                    data-modal-hide="detailModal{{ $pinjam->id }}"></div>
                ...
                <div class="relative z-[70] w-full max-w-2xl mx-auto px-4">
                    ...
                    <div class="relative bg-white rounded-lg shadow max-h-[90vh] flex flex-col">
                        <!-- Fixed Header -->
                        <div
                            class="flex items-center justify-between p-4 md:p-5 border-b rounded-t sticky top-0 bg-white z-10">
                        ...
                        <div class="flex-1 overflow-y-auto p-4 md:p-5 space-y-4">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                ...
                                <div class="space-y-2">
                                    @foreach ($pinjam->tanggalPeminjaman as $tanggal)
                                        <div
                                            class="flex items-center text-sm bg-white p-2 rounded-lg border border-gray-100">
                                            <span class="font-medium text-gray-600">{{ $tanggal->tanggal }}</span>
                                            <span class="mx-2 text-gray-400">|</span>
                                            <span class="text-gray-500">{{ $tanggal->jadwal->mulai }} -
                                                {{ $tanggal->jadwal->selesai }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                ...
                                <div class="space-y-3">
                                    <a href="{{ asset('storage/' . $pinjam->suratPeminjaman) }}" target="_blank"
                                        ...
                                        <span
                                            class="ml-3 text-sm font-medium text-gray-600 group-hover:text-gray-900">Surat
                                            Peminjaman</span>
                                    </a>
                                    <a href="{{ asset('storage/' . $pinjam->rundown) }}" target="_blank"
                                        ...
                                        <span
                                            class="ml-3 text-sm font-medium text-gray-600 group-hover:text-gray-900">Rundown
                                            Acara</span>
                                    </a>
                                </div>
                            </div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div class="mb-6 p-4 bg-white rounded-lg border border-yellow-200">
                                        <div class="flex justify-between items-center mb-4">
                                            ...
                                            <span
                                                class="text-2xl font-bold text-gray-900">Rp{{ number_format($pinjam->totalTarif, 0, ',', '.') }}</span>
                                        </div>
                            ...
                            @if ($pinjam->status == 'ditolak')
                                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                        <svg class="w-5 h-5 mr-2 text-red-500" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                        </svg>
                                    ...
                                    <div class="space-y-3">
                                        <div class="p-3 bg-white rounded-lg border border-gray-200">
                                            <p class="text-sm font-medium text-gray-600">{{ $pinjam->feedbackPenolakan }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if (
                                ($pinjam->status == 'diajukan' && $pinjam->alasanTolakBatal) ||
                                    ($pinjam->status == 'disetujui' && $pinjam->alasanTolakBatal))
                                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                        <svg class="w-5 h-5 mr-2 text-red-500" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="space-y-3">
                                        <div class="p-3 bg-white rounded-lg border border-gray-200">
                                            <p class="text-sm font-medium text-gray-600">{{ $pinjam->alasanTolakBatal }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($pinjam->status == 'diajukanbatal')
                                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div class="space-y-3">
                                        <div class="p-3 bg-white rounded-lg border border-gray-200">
                                            <p class="text-sm font-medium text-gray-600">{{ $pinjam->alasanPembatalan }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if ($pinjam->canBeCancelled())
                                ...
                                <div id="cancellationModal{{ $pinjam->id }}"
                                    ...
                                    <div class="min-h-screen px-4 text-center flex items-center justify-center">
                                        ...
                                        <div
                                            ...
                                            <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">Konfirmasi
                                                Pembatalan</h3>
                                            ...
                                            <form action="{{ route('peminjaman.cancel', $pinjam->id) }}" method="POST"
                                                ...
                                                <div class="flex items-center justify-end space-x-3">
                                                    <button type="button"
                                                        onclick="hideCancellationForm('{{ $pinjam->id }}')"
